$(document).ready(function(){
    ...
    $("#selectclubcomposer").on('change',function(){
        // salert("hello");
        var action = 'data2';
        var composer =  $("#selectclubcomposer").val();
        var club = $("#selectclubb").val();
       

        $.ajax({

            url: "../web_services/filterclubtrack.php",
            method: "POST",
            data: {
                action: action,
                composer: composer,
            ...
            success: function(response) {
                // alert(response);
                $("#displayclubtracks").html(response);
                // $("#loader").hide();
                // $("#textChange").text("Filtered Products");
